...
class JugsStates {
    ...
    move(direction) {
        ...
        let newstate = new JugsStates([leftwater, rightwater], this.goal_state, this.volume);
        return newstate;
    }

    printJugs(isheuri = false) {
        console.log(`[${this.current_state[0]} , ${this.current_state[1]}]`);
        if (isheuri)
            console.log("HEURISTIC: " + this.heuristic());
        else
            console.log("------");
    }

    heuristic() {
        const currentState = this.current_state;
        const difference = Math.abs(this.goal_state[0] - currentState[0]) + Math.abs(this.goal_state[1] - currentState[1]);
        return difference;
    }
    ...
    convert() {
        return 10 * this.current_state[0] + this.current_state[1];
    }
}
...
let state = new JugsStates(...k);

console.log("============ ORDER OF NODES VISITED ===============");
let ans = greedybest(state);
console.log("==========================================");

let finalstate = new JugsStates(...k)
